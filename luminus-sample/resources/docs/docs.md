...
The `home-routes` handler in the `luminus-sample.routes.home` namespace
...
The `home-page` function will in turn call the `luminus-sample.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `luminus-sample.handler` namespace:
...
Middleware used for development is placed in the `luminus-sample.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `luminus-sample.core` in some other namespace.
